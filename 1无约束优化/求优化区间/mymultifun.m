function out=mymultifun(phi,x_range,x0,x_syms,type)   %求多元函数的极值
syms lamda
switch type
    case 'u_L'  %根据特征值求极值
        num=length(x_syms);
        for i=1:num
          d{i}=diff(phi,x_syms{i});
          for j=1:num
            H(i,j)=diff(d{i},x_syms{j});
          end
        end
        ...
        fun=['[',char(d{1}),','];
        for i=2:num-1
          fun=strcat(fun,char(d{i}),',');
        end
        fun=strcat(fun,char(d{num}),']');
        x0=solve(fun);   %驻点
        names=fieldnames(x0)';
        for i=1:num
    ...
    case 'u_u'
      ...
      for i=1:num
         ...
         for j=1:num
           if findstr(s{i},x_syms{j})
             s{i}=strrep(s{i},x_syms{j},['x(',num2str(j),')']);
           end
         end
         if findstr(s{i},'l')
